...
os.makedirs("images_g", exist_ok=True)
os.makedirs("models_g", exist_ok=True)
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:25], "images_g/%d.png" % batches_done, nrow=5, normalize=True)
            torch.save(generator.state_dict(), 'models_g/model_G_%d.pth' % batches_done)
            torch.save(discriminator.state_dict(), 'models_g/model_D_%d.pth' % batches_done)
    ...
    with open('time/dcgan_time_g.txt', 'a') as f:
